 ...
 *
 * 两个分组，运动规则不同：
 * - wheel：绕视轴缓慢**轮转**（整片天空一起转，月亮因此自己就会移动，不需要额外做动画），
 *   同时受滚动 travel 驱动 —— 滚一页 ≈ 转过 16°，不滚动时也有极缓慢的自转。
 * - horizon：低云与卫星，**不跟着转**（云贴着地平线、卫星走自己的直线）。
 *
...
void main() {
  ...
  float earthshine = 0.055 * (1.0 - lit);
...
export function createSkyDome(
  ...
  // ── 月亮：唯一强锚点。它不需要自己的动画 —— 跟着 wheel 轮转就会划过天空
  const moonMat = mat(MOON_FRAG, { uOpacity: { value: 1 }, uSun: { value: new THREE.Vector2(-0.42, 0.3) } }, false);
  const moon = new THREE.Mesh(quad, moonMat);
  moon.scale.setScalar(viewH * 0.05);
  moon.position.set(viewH * 0.3, viewH * 0.26, 0);
  wheel.add(moon);
  ...
  halo.scale.setScalar(viewH * 0.34);
  ...
  return {
    ...
    update(step, elapsed, aspect, pointerX, pointerY, travel) {
      ...
      haloMat.uniforms.uOpacity.value = 0.2 + 0.08 * Math.sin(elapsed * 0.07);
